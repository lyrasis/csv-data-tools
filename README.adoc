...
LibreOffice macro library.

See the "Import a Library" section in https://help.libreoffice.org/latest/en-US/text/sbasic/shared/01030400.html?DbPAR=BASIC[the LibreOffice Organizing Libraries and Modules documentation].

Currently the library contains the following:

* Module1 (default macro module)
** Main (default base macro; doesn't actually do anything)
** PrepCSV - Applies Autofilter (makes all columns filterable); Freezes the header row; Changes width of all columns to 1

Recommended: Assign a keyboard shortcut to the PrepCSV macro and hit it as soon as the file opens.
How to assign keyboard shortcut:

* Tools > Macros > Organize Macros > Basic
* Navigate to/select PrepCSV
* Click the "Assign" button
* Navigate to/select the PrepCSV macro again in the Category/Function columns at the bottom of the dialog
* Find an unused shortcut in the top pane
* Click the Assign button
...
Safely opens a CSV file with all columns as text data in LibreOffice Calc.

While this can be used from terminal, the idea is that this can be used as an automation or shortcut to open CSVs without having to deal with the import dialog.
That dialog's settings, *EXCEPT FOR THE COLUMN FORMATS*, are now sticky, but the column formats are the most important part for not having weird things happen to data when you open a CSV.

Exactly how you would automate this depends on your file opening habits.

Requires the `soffice` command to be available in your PATH. You can do the following in your terminal config to make this happen:

[source,bash]
----
# Use LibreOffice on commandline
PATH="$PATH:/Applications/LibreOffice.app/Contents/MacOS"
----

Assumes CSV is UTF-8, with comma field separator and double-quote text delimiter.
...
[#split]
=== Split CSVs with header included in each smaller file
...
[source,bash]
----
xan split -S 50000 -f smaller_{}.csv big_file.csv
----

* Smaller files will have 50000 rows. (Last file may have fewer)
* Smaller files will be named `smaller_{}.csv`, where row number of orig file is inserted as `{}`.
...
[source,bash]
----
split -l 10000 -d file.csv file_
----
...
[#x_y_when_x_has_multi_y]
=== Show X and Y values when X value is associated with multiple unique Y values

This is a thing I have done frequently in the past with a pivot table, plus some copy/pasting and sorting of the pivot table.

Using xan for it is MUCH faster.

This technique answers questions like:

* I have normalized these location values; what normalized values had more than one original form collapsed into them, and what were those normalized forms?
* What `recordStatus` values are used/paired with more than one `descriptionLevel` value, and what `descriptionLevel` values are used for these `recordStatus` values? (We expect pairs like: `recordStatus` = "complete" and `descriptionLevel` = "full",  and `recordStatus` = "prelim" and `descriptionLevel` = "core", but need to see if there are more complex/paradoxical relationships between these values)

With a file (location-local.csv) having columns:

[source,bash]
----
❯ xan headers location-local.csv
0 location
1 norm
2 termdisplayname
----
...
[NOTE]
====
The above is the result of applying the following kiba-extend transforms to a source with a `location` column:
...
transform Replace::NormWithMostFrequentlyUsedForm,
  ...
  target: :termdisplayname
----

location:: the original value
norm:: normalized version of location
termdisplayname:: most frequently used form of `location` for rows normalized to `norm`
====
...
[source,bash]
----
xan select location,termdisplayname location-local.csv \
| xan dedup -s location,termdisplayname \
| xan window -g termdisplayname 'count() as unique_loc_count' \
| xan filter 'unique_loc_count > 1' \
| xan select location,termdisplayname \
| xan v
----
...
....
┌───┬──────────────────────┬──────────────────────┐
│ - │ location             │ termdisplayname      │
├───┼──────────────────────┼──────────────────────┤
│ 0 │ ANNEX SHELF 126B     │ ANNEX SHELF 126B     │
│ 1 │ ANNEX SHELF 126 B    │ ANNEX SHELF 126B     │
│ 2 │ B-01 SHELF 102E      │ B-01 SHELF 102E      │
│ 3 │ B-01 SHELF 26B BOX 9 │ B-01 SHELF 26B BOX 9 │
│ 4 │ B-01 SHELF 26B BOX9  │ B-01 SHELF 26B BOX 9 │
│ 5 │ B-01 Shelf 102E      │ B-01 SHELF 102E      │
│ 6 │ SHELF 128D BOX 325   │ SHELF 128D BOX 325   │
│ 7 │ SHELF 128D, BOX 325  │ SHELF 128D BOX 325   │
└───┴──────────────────────┴──────────────────────┘
....
...
Some of the scripts here assume csvkit is installed. We have been using this tool for a long time for various CSV purposes.

Python-based tool.

https://csvkit.readthedocs.io/en/latest/index.html[docs]

https://github.com/lyrasis/data-migration-docs/blob/main/tech_setup.adoc#csvkit[Lyrasis Migrations Team approved installation instructions]

[#xan]
=== xan

Rust-based tool. Ridiculously fast.

Kristina just recently discovered this one (2026-07) and is testing it out/getting used to it.

https://github.com/medialab/xan[repo/docs]

Installation:

`mise use -g rust` (If you do not already have Rust installed. To check: `which rustup`)

`cargo install xan --locked`

.RIDICULOUSLY FAST
....
❯ time csvstat --count movements_common.csv
822726
...
❯ time xan count movements_common.csv
822726
...
❯ time csvstat -c 3 movements_common.csv
  3. "reasonformove"

	Type of data:          Text
	Contains null values:  True (excluded from calculations)
	Non-null values:       21521
	Unique values:         8
	Longest value:         20 characters
	Most common values:    None (801205x)
	                       New Storage Location (9203x)
	                       Inventory (6945x)
	                       Exhibition (2481x)
	                       Photography (1342x)
...
❯ time xan frequency -s reasonformove movements_common.csv | xan view

Displaying 3 cols from 8 rows of <stdin>
┌───┬───────────────┬──────────────────────┬────────┐
│ - │ field         │ value                │ count  │
├───┼───────────────┼──────────────────────┼────────┤
│ 0 │ reasonformove │ <empty>              │ 801205 │
│ 1 │ reasonformove │ New Storage Location │   9203 │
│ 2 │ reasonformove │ Inventory            │   6945 │
│ 3 │ reasonformove │ Exhibition           │   2481 │
│ 4 │ reasonformove │ Photography          │   1342 │
│ 5 │ reasonformove │ Conservation         │    716 │
│ 6 │ reasonformove │ Loan                 │    607 │
│ 7 │ reasonformove │ Research             │    227 │
└───┴───────────────┴──────────────────────┴────────┘
...
❯ time csvgrep -c 8 -m 2023 movements_common.csv | csvlook --max-rows 10 | less -S

real	0m1.945s
user	0m1.374s
sys	0m0.100s

❯ time xan search -s plannedremovaldate 2023 movements_common.csv | xan view -p
...
`xan view -p filename.csv` is like `csvlook --max-rows 10 filename | less -S` with less typing.

Includes `xan sample` command, with option 🤣:
...
But, in general, the sample command _could_ be useful for quotes or something?

Considers .jsonl (new-line delimited JSON records) as "csv" format and can handle them to some extent. I have not tested this yet.
